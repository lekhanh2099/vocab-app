...
export function AppSelect(props: {
  ...
  return <Select<string>
      ...
      itemComponent={(itemProps) => {
        ...
        >
          <div class="min-w-0 flex-1">
            ...
            <Show when={option()?.description}><div class="mt-0.5 truncate text-xs font-medium text-slate-400">{option()?.description}</div></Show>
          </div>
    ...
    >
      <Select.Label class="text-sm font-bold text-slate-600">{props.label}</Select.Label>
      <Select.Trigger
...
export function PageHero(props: ParentProps & { eyebrow?: string; title: JSX.Element; description?: JSX.Element; actions?: JSX.Element }) {
  return <section class={`${surface} p-5 sm:p-6 lg:p-7`}>
    <Show when={props.eyebrow}><div class="text-xs font-extrabold uppercase tracking-[0.12em] text-blue-700">{props.eyebrow}</div></Show>
...
export function SectionHeader(props: { title: JSX.Element; meta?: JSX.Element; description?: JSX.Element }) {
  return <div class="mt-6 flex items-end justify-between gap-4 px-0.5">
    <div class="min-w-0"><h2 class="text-lg font-black tracking-[-0.02em] text-slate-900 sm:text-xl">{props.title}</h2><Show when={props.description}><div class="mt-1.5 text-sm leading-5 text-slate-500">{props.description}</div></Show></div>
    <Show when={props.meta}><div class="shrink-0 text-sm font-semibold tabular-nums text-slate-500">{props.meta}</div></Show>
  </div>;
}

export function Field(props: ParentProps & { label: string }) {
  return <label class="grid min-w-0 gap-1.5 text-sm font-bold text-slate-600"><span>{props.label}</span>{props.children}</label>;
}
...
export function Badge(props: ParentProps & { tone?: "neutral" | "blue" | "green" | "red" | "amber" }) {
  ...
  return <span class={`inline-flex min-h-7 items-center rounded-full px-2.5 text-xs font-extrabold ${tone()}`}>{props.children}</span>;
}
