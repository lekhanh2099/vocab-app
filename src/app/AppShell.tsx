    ...
    <div class={`min-h-dvh bg-[#f7f7f5] text-slate-900 ${isGameSession() ? "" : "pb-[calc(4.75rem_+_env(safe-area-inset-bottom))] md:pb-0"}`}>
      <Show when={!isGameSession()}>
        <header class="sticky top-0 z-40 border-b border-slate-200/80 bg-[#f7f7f5]/95 pt-[env(safe-area-inset-top)] backdrop-blur-xl">
          <div class="mx-auto flex min-h-16 w-full max-w-[80rem] items-center gap-4 px-3 sm:px-4 lg:px-6">
            <A href="/" class="flex min-w-0 items-center gap-2.5 no-underline" end><img src="/icons/icon-192.png" alt="" class="size-9 shrink-0 rounded-xl shadow-sm"/><span class="min-w-0"><b class="block truncate text-lg font-black tracking-[-0.03em] text-slate-900">Vocab Universe</b><span class="mt-0.5 hidden text-xs text-slate-500 sm:block">Solid · FSRS · offline-first</span></span></A>
            <nav class="ml-auto hidden items-center gap-1 md:flex" aria-label="Điều hướng chính desktop">{navItems.map((item) => {
              const Icon = item.icon;
              return <A href={item.href} end={item.href === "/"} activeClass="bg-blue-50 text-blue-700" class="inline-flex min-h-10 items-center gap-2 rounded-xl px-3 text-sm font-extrabold text-slate-600 no-underline transition-colors hover:bg-slate-100"><Icon size={17} strokeWidth={2.2}/>{item.label}</A>;
            })}</nav>
            <A href="/settings" class="grid size-11 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white text-slate-700 no-underline shadow-sm transition-colors hover:border-slate-300 hover:text-slate-950" aria-label="Cài đặt"><Settings size={19} strokeWidth={2.2}/></A>
          </div>
        </header>
      </Show>

      <main class={isGameSession() ? "w-full" : "mx-auto w-full max-w-[80rem] px-3 py-3 sm:px-4 sm:py-4 lg:px-6 lg:py-5"}>
      ...
      <Show when={!isGameSession()}>
        <nav class="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200 bg-white/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl md:hidden" aria-label="Điều hướng chính mobile"><div class="mx-auto grid max-w-2xl grid-cols-5 gap-1 px-2 py-1.5">{navItems.map((item) => {
          const Icon = item.icon;
          return <A href={item.href} end={item.href === "/"} activeClass="bg-blue-50 text-blue-700" class="flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl text-xs font-bold text-slate-500 no-underline"><Icon size={19} strokeWidth={2.2}/><span>{item.label}</span></A>;
        })}</div></nav>
      </Show>
